...
#                  message(STATUS "Appending lib ${_CURRENT_LIB} as ${_importedLocation}")
                  ...
                  if(_linkInterfaceLibs)
                     foreach(_currentLinkInterfaceLib ${_linkInterfaceLibs})
                        if("${_currentLinkInterfaceLib}" MATCHES "\\\$<\\\$<NOT:\\\$<CONFIG:([^>]*)>>:([^>]*)>")
                           if(NOT CMAKE_MATCH_1 STREQUAL _importedConfigToUse)
                              set(_currentLinkInterfaceLib ${CMAKE_MATCH_2})
                           else()
                              set(_currentLinkInterfaceLib)
                           endif()
                        elseif("${_currentLinkInterfaceLib}" MATCHES "\\\$<\\\$<CONFIG:([^>]*)>:([^>]*)>")
                           if(CMAKE_MATCH_1 STREQUAL _importedConfigToUse)
                              set(_currentLinkInterfaceLib ${CMAKE_MATCH_2})
                           else()
                              set(_currentLinkInterfaceLib)
                           endif()
                        elseif("${_currentLinkInterfaceLib}" MATCHES "\\\$<LINK_ONLY:([^>]*)>")
